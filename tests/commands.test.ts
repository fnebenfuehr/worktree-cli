/**
 * Integration tests for CLI commands (src/commands/*.ts)
 * Tests high-level command behavior including validation, git operations, and error handling
 */
...
describe('cloneCommand validation', () => {
...
describe('removeCommand validation', () => {
	test('throws ValidationError when no branch provided', async () => {
...
describe('switchCommand validation', () => {
	test('throws ValidationError when no branch provided', async () => {
...
describe('listCommand behavior', () => {
	test('returns 0 exit code when called in git repo', async () => {
	...
	test('throws ValidationError when not in git repository', async () => {
		...
		try {
			process.chdir('/tmp');
			await expect(listCommand()).rejects.toThrow(ValidationError);
			await expect(listCommand()).rejects.toThrow('Not inside a git repository');
		} finally {
			process.chdir(originalDir);
		}
	});
